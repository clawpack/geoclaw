...
class FGoutGrid(object):
    ...
    def __init__(self,fgno,outdir='_output',output_format='binary'):
        ...
        plotdata.file_prefix = 'fgout%s' % str(fgno).zfill(4)
        if format=='binary':
    ...
    def read_frame(self, frameno):
        ...
        print('Reading fgout frame %i from %s' % (frameno,self.plotdata.outdir))
        try:
        ...
        fgout_frame.h = q[0,:,:]
        fgout_frame.eta = q[3,:,:]
        fgout_frame.B = fgout_frame.eta - fgout_frame.h
        
        hu = q[1,:,:]
        hv = q[2,:,:]
        fgout_frame.u = numpy.where(fgout_frame.h>0.01, hu/fgout_frame.h, 0.)
        fgout_frame.v = numpy.where(fgout_frame.h>0.01, hv/fgout_frame.h, 0.)
        fgout_frame.s = numpy.sqrt(fgout_frame.u**2 + fgout_frame.v**2)
        
        fgout_frame.X = patch.grid.p_centers[0]
        fgout_frame.Y = patch.grid.p_centers[1]
        #print('+++ shapes: ',fgout_frame.X.shape,fgout_frame.Y.shape,\
        #    fgout_frame.h.shape)
