    ...
    def read_atcf(self, path, verbose=False):
        ...
        df = pd.read_csv(path, engine="python", sep=",+", names=[
            ...
            converters={
                ...
                "CY": lambda d: (d if isinstance(d, int) else np.nan),
                "VMAX": lambda d: (d if isinstance(d, float) else np.nan),
                "MSLP": lambda d: (d if isinstance(d, float) else np.nan),
                "RAD": lambda d: (d if isinstance(d, float) else np.nan),
                "RAD1": lambda d: (d if isinstance(d, float) else np.nan),
                "RAD2": lambda d: (d if isinstance(d, float) else np.nan),
                "RAD3": lambda d: (d if isinstance(d, float) else np.nan),
                "RAD4": lambda d: (d if isinstance(d, float) else np.nan),
                "ROUTER": lambda d: (d if isinstance(d, float) else np.nan),
                "RMW": lambda d: (d if isinstance(d, float) else np.nan),
            },
            ).fillna(value = {
                "BASIN": '',
                "CY": np.nan,
