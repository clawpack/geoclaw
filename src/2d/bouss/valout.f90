...
subroutine valout(level_begin, level_end, time, num_eqn, num_aux)
    ...
    real(kind=8) :: h, hu, hv, eta, zero, huc, hvc
            ...
            if (output_format == 1) then
                    ...
                    do j = num_ghost + 1, num_cells(2) + num_ghost
                        do i = num_ghost + 1, num_cells(1) + num_ghost
                            ...
                            if (num_eqn == 5) then
                                zero = 0.d0 ! dummy val for components 4,5
                                huc = alloc(iadd(4, i, j))
                                hvc = alloc(iadd(5, i, j))
                                write(out_unit, "(50e26.16)") h, hu, hv, &
                                                              huc, hvc, eta
                            else
                                ! standard geoclaw output:
