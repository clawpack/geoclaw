...
c
#ifdef WHERE_AM_I
      write(*,*)"starting setgrd"
#endif

      dtinit = possk(1)
c     if (mxnest .eq. 1) go to 99
c
      levnew =  2
      time   = start_time
c
...
c     set deepest bathy for each grid
      if (ibouss .gt. 0) then
        do lev = 1, lfine
           numBoussGrids = 0 ! get ready to count them
           minfo => matrix_info_allLevs(lev)   ! entry for this level
           mptr = lstart(lev)
 74           continue
            ...
            !call setMaxMinDepth(alloc(locaux),mptr,naux,nx,ny,nghost)
              if (lev.ge.minLevelBouss .and. lev.le.maxLevelBouss) then
...
c     set up for Bouss matrices
      if (ibouss .gt. 0) then
        newGrids = .true.  ! for all levels, indicates first time thru for initializations
        numTimes = 0  ! counts number of calls to Petsc per level
        itcount = 0   ! will count number of iterations per Petsc call per level
        if (minLevelBouss .le. mxnest .and. ibouss .gt. 0) then  ! 
           mtype = 11
           solver = 0
           do lev = 1, lfine
             if (lev.le.maxLevelBouss .and. lev.ge.minLevelBouss) then
                ...
                ! set up for linear system solver
                if (isolver .eq. 2) then
#ifdef HAVE_PARDISO
                  call pardisoinit(pt(1,lev),mtype,solver,iparm,dparm,
     &                            error)
                  write(*,*)"setgrid initialized pardiso for lev ",lev
#endif
                 endif
             endif
           end do
