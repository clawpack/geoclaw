...
program amr2
    ...
    use amr_module, only: timeLinSolve, timeLinSolveCPU
    ...
    !! for pardiso initializations
#ifdef HAVE_PARDISO
    use bouss_module, only: pt,maxfct,mnum,mtype,phase,nn,ddum,idum
    use bouss_module, only: nrhs,iparm,msglvl,error,dparm
#endif
    use bouss_module, only: isolver, minLevelBouss, maxLevelBouss
    use bouss_module, only: itcount, numTimes
...
!   needs to be very first line in main
    ...
    read(inunit,*) memsize  ! initialsize of alloc array
    ...
    if (rest) then
        ...
        ! need these before set-gauges so num_out_vars set right for gauges
...
!$   maxthreads = omp_get_max_threads() 
    ...
    if (isolver .eq. 2) then
      ...
      do level = 1, mxnest
         if (level .le. maxLevelBouss .and. level .ge. minLevelBouss)    &
         CALL pardiso (pt(1,level),maxfct,mnum,mtype,phase,nn,ddum,idum, &
                       idum,idum,nrhs,iparm,msglvl,ddum,ddum,error,dparm)
      end do
#endif
    endif
    ...
    !Integration time
    format_string="('Integration Time (stepgrid + BC + implicit solve + overhead)')"
    write(timing_unit,format_string)
    write(*,format_string)
    ...
    !Linear Solve
    !format_string="('linear solve  ',1f15.3,'        ',1f15.3,'    ',e17.3)"
    format_string="('Linear solve  ',1f15.3,'        ',1f15.3)"
    write(timing_unit,format_string) &
    ...
    if (isolver .eq. 3) then
       ...
       do level = 1, mxnest
         if (numTimes(level) .gt. 0) then
           ...
           write(timing_unit,989) level, numTimes(level), real(itcount(level))/real(numTimes(level))
 989       format(18x, i10,2x,i10,3x,f10.2)
         else
           exit
         endif
       end do
...
!         write(*,*) "No implicit solves performed"
!         write(*,*)
!     else
!         write(*,*)"avg # iterative refinements = ",avgIterRef/countIterRef
!         write(*,*)"max # iterative refinements = ",maxIterRef
!         write(*,format_string)
!         write(*,*)
!     endif
    endif
