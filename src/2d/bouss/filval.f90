        ...
        do i=2, mic-1
               ...
               do ico = 1,refinement_ratio_x
                    ...
                    if (setflags(ifine,jfine) .eq. NEEDS_TO_BE_SET) then
                       ...
                       if (val(1,ifine,jfine) <= dry_tolerance) then
                          ...
                          if (nvar .eq. 5) then
                            val(4,ifine,jfine) = 0.d0
                            val(5,ifine,jfine) = 0.d0
                          endif
                       endif
            ...
            if (finemass >= dry_tolerance) then
                do ivar = 2,nvar
                    ...
                    if (ivar .lt. 4) then ! setting momentum, not correction
                        ...
                        do jco = 1,refinement_ratio_y
                            ...
                            if (ivar .lt. 4) then
                                ...
                                if (vf > velmax .or. vf < velmin) then
                                    fineflag(ivar) = .true.
                                    ! in this case momentum set in next loop
                                    exit
                                else
                                    val(ivar,ifine,jfine) = hvf
                                endif
                            else  ! just set correction vars 4,5 directly
                                val(ivar,ifine,jfine) = hvf
                            endif
                        enddo
                    enddo
                    ...
                    if (ivar .lt. 4) then !this only applies to vel vars
                        ...
                        ! ivar = 4 or 5, reset correction to 0 if 
                        ! momentum had to be corrected.  Is this right?
                        if (fineflag(1) .or. fineflag(ivar-2)) then
                           do jco = 1,refinement_ratio_y
                           do ico = 1,refinement_ratio_x
                               jfine = (j-2) * refinement_ratio_y + nghost + jco
                               ifine = (i-2) * refinement_ratio_x + nghost + ico
