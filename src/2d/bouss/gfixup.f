...
c  so I didnt want to have to deal with it
       ...
       subroutine prepnewgrids_new(listnewgrids,num,level)

       use amr_module
       implicit double precision (a-h,o-z)
       integer listnewgrids(num),cost(num),index(num),prevptr

       mptr = newstl(level)
       do j = 1, num
          listnewgrids(j) = mptr
          cost(j) = (node(ndihi,mptr)-node(ndilo,mptr)+3)*
 ...
 100   format(20i3)
       ...
       ! from end. grids stay in place, just their ptrs change
       newstl(level) = listnewgrids(index(num))
       prevptr = listnewgrids(index(num))
       do i = 1, num-1
          node(levelptr, prevptr) = listnewgrids(index(num-i))
          prevptr = listnewgrids(index(num-i))
       end do
       node(levelptr, prevptr) = null ! signal last grid
