...
!
    ...
    real(kind=8) :: faddm(1-mbc:maxm+mbc,meqn)
    real(kind=8) :: faddp(1-mbc:maxm+mbc,meqn)
    real(kind=8) :: gaddm(1-mbc:maxm+mbc,meqn,2)
    real(kind=8) :: gaddp(1-mbc:maxm+mbc,meqn,2)
...
!     real(kind=8) :: bpadq(meqn,1-mbc:maxm + mbc)
    ...
    do j = 0,my+1
        ...
        ! Update fluxes
        do i=1,meqn
            fm(i,1:mx+1,j) = fm(i,1:mx+1,j) + faddm(1:mx+1,i)
            fp(i,1:mx+1,j) = fp(i,1:mx+1,j) + faddp(1:mx+1,i)
            gm(i,1:mx+1,j) = gm(i,1:mx+1,j) + gaddm(1:mx+1,i,1)
            gp(i,1:mx+1,j) = gp(i,1:mx+1,j) + gaddp(1:mx+1,i,1)
            gm(i,1:mx+1,j+1) = gm(i,1:mx+1,j+1) + gaddm(1:mx+1,i,2)
            gp(i,1:mx+1,j+1) = gp(i,1:mx+1,j+1) + gaddp(1:mx+1,i,2)
         end do
    ...
    do i = 0, mx+1
        ...
        ! Update fluxes
        do j=1,meqn
            gm(j,i,1:my+1) = gm(j,i,1:my+1) + faddm(1:my+1,j)
            gp(j,i,1:my+1) = gp(j,i,1:my+1) + faddp(1:my+1,j)
            fm(j,i,1:my+1) = fm(j,i,1:my+1) + gaddm(1:my+1,j,1)
            fp(j,i,1:my+1) = fp(j,i,1:my+1) + gaddp(1:my+1,j,1)
            fm(j,i+1,1:my+1) = fm(j,i+1,1:my+1) + gaddm(1:my+1,j,2)
            fp(j,i+1,1:my+1) = fp(j,i+1,1:my+1) + gaddp(1:my+1,j,2)
        end do
