...
if __name__ == "__main__":
    ...
    for i, t in enumerate(T):
        soln = Solution(os.getcwd()+"/_output", i+1, min_lvl=3)
        print(soln)

        print("T =", t)
        print("\tValid Max AMR level: ", soln.max_lvl)

        n_cells = numpy.zeros(soln.max_lvl, dtype=numpy.int)
        volumes = numpy.zeros(soln.max_lvl, dtype=numpy.float64)

        for p in soln.patches:
            if p.valid:
                n_cells[p.AMR_level-1] += (p.mx * p.my)
                volumes[p.AMR_level-1] += (numpy.sum(p.data[:, :, 0]) * p.dx * p.dy)
        ...
        fig = pyplot.figure()
        ax = fig.gca()
        im = soln.plot_at_axes(ax, soln.max_lvl, vmax=0.005, cmap=pyplot.cm.jet)
        ...
        pyplot.savefig("T={}s.png".format(t))
        #pyplot.show()
