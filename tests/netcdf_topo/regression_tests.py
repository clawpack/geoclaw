...
class NetCDFBowlSloshTest(test.GeoClawRegressionTest):
    ...
    def setUp(self):
        ...
        try:
            ...
            with netCDF4.Dataset(this_path,'r') as f:
                x = numpy.array(f.variables['lon'])
                y = numpy.array(f.variables['lat'])
                Z = numpy.array(f.variables['elevation'])

            with netCDF4.Dataset(this_path,'w') as out:
                lat = out.createDimension('lat',len(y))
                lon = out.createDimension('lon',len(x))
